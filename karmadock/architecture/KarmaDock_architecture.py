...
class KarmaDock(nn.Module):
    ...
    def cal_rmsd_sym(self, pos_ture, pos_pred, batch_sym_index, batch, if_r=True):
        sym_index_len_size = np.asarray(
            [sym_index.shape for sym_index in batch_sym_index]
        )
        ...
        batch_sym_index = [
            (
                np.concatenate(
                    [
                        batch_sym_index[idx],
                        np.arange(i[1]).reshape((1, -1)).repeat(sym_max - i[0], axis=0),
                    ],
                    axis=0,
                )
                + sym_index_len_size[:idx][:, 1].sum()
                if i[0] < sym_max
                else batch_sym_index[idx] + sym_index_len_size[:idx][:, 1].sum()
            )
            for idx, i in enumerate(sym_index_len_size)
        ]
        ...
        return (
            torch.stack(
                [
                    ...
                    for sym_index in batch_sym_index
                ],
                dim=0,
            )
            .min(dim=0)
    ...
    def forward(self, data, device, pos_r, recycle_num=3):
        # mdn aux labels
        atom_types_label = torch.argmax(
            data["ligand"].node_s[:, :18], dim=1, keepdim=False
        )
        bond_types_label = torch.argmax(
            data["ligand", "l2l", "ligand"].edge_s[data["ligand"].cov_edge_mask][:, :5],
            dim=1,
            keepdim=False,
        )
        # encoder
        ...
        lig_node_s = self.lig_encoder(
            ...
            data["ligand", "l2l", "ligand"]
            .edge_s[data["ligand"].cov_edge_mask]
            .to(torch.float32),
        ...
        data["protein"].node_s, data["ligand"].node_s = (
            node_s[:pro_nodes],
            node_s[pro_nodes:],
        )
        ...
        if pos_r:
            ...
            for re_idx in range(recycle_num):
                ...
                for layer in self.egnn_layers:
                    ...
                    rmsd_losss = torch.cat(
                        [
                            rmsd_losss,
                            self.cal_rmsd(
                                pos_ture=data["ligand"].xyz,
                                pos_pred=pos[pro_nodes:],
                ...
                node_s = self.node_gate_layer(
                    torch.cat([data["protein"].node_s, data["ligand"].node_s], dim=0),
                    node_s,
                )
            ...
            rmsd_losss = (
                rmsd_losss[:, 8 * count_idx : 8 * (count_idx + 1)].mean(dim=1)
                + rmsd_losss[:, -1]
            ).mean()
        else:
        ...
        pi, sigma, mu, dist, _, atom_types, bond_types = self.mdn_layer(
            lig_s=lig_node_s,
            lig_pos=lig_pos_,
            ...
            edge_index=data["ligand", "l2l", "ligand"].edge_index[
                :, data["ligand"].cov_edge_mask
            ],
        )
        try:
            mdn_loss_true = self.mdn_layer.mdn_loss_fn(pi, sigma, mu, dist)
            mdn_loss_true = (
                mdn_loss_true[torch.where(dist <= self.mdn_layer.dist_threhold)[0]]
                .mean()
                .float()
            )
    ...
    def ligand_docking(
        self, data, docking=False, scoring=False, recycle_num=3, dist_threhold=5
    ):
        """
