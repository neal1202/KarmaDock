...
argparser.add_argument(
    "--mode", default="vs", help="virtual screening on GPU or generating graph on CPUs"
)
argparser.add_argument(
    "--ligand_smi",
    type=str,
...
argparser.add_argument(
    "--graph_dir", type=str, default="/root/graphs", help="the dir for saving graph"
)
argparser.add_argument(
    "--out_dir",
    type=str,
...
argparser.add_argument("--random_seed", type=int, default=2020, help="random_seed")
argparser.add_argument(
    "--out_init",
    action="store_true",
    default=False,
    help="whether to save initial poses to sdf file",
)
argparser.add_argument(
    "--out_uncoorected",
    action="store_true",
    default=False,
    help="whether to save uncorrected poses to sdf file",
)
argparser.add_argument(
    "--out_corrected",
    action="store_true",
    default=False,
    help="whether to save corrected poses to sdf file",
)
...
# test
test_dataset = VSTestGraphDataset_FlyReload_SMI(
    protein_file=pocket_file, ligand_path=vs_libray_path, pocket_center=pocket_center
)
if args.mode != "vs":
    ...
    # dataloader
    test_dataloader = PassNoneDataLoader(
    ...
    if len(test_dataset) > 0:
        ...
        with torch.no_grad():
            for idx, data in enumerate(tqdm(test_dataloader, desc="prediction")):
                ...
                lig_pos, _, _ = model.module.docking(
                    pro_node_s, lig_node_s, data, recycle_num=3
                )
                ...
                poses, _, _ = correct_pos(
                    data,
                    mask=mdn_score_pred <= score_threshold,
                    out_dir=args.out_dir,
                    out_init=args.out_init,
                    out_uncoorected=args.out_uncoorected,
                    out_corrected=args.out_corrected,
                )
                ff_corrected_pos = torch.from_numpy(
                    np.concatenate([i[0] for i in poses], axis=0)
                ).to(my_device)
                align_corrected_pos = torch.from_numpy(
                    np.concatenate([i[1] for i in poses], axis=0)
                ).to(my_device)
                mdn_score_pred_ff_corrected = model.module.scoring(
                    lig_s=lig_node_s,
                ...
                mdn_score_pred_align_corrected = model.module.scoring(
                    lig_s=lig_node_s,
                ...
                pki_scores_pred_ff = torch.cat(
                    [pki_scores_pred_ff, mdn_score_pred_ff_corrected], dim=0
                )
                pki_scores_pred_aligned = torch.cat(
                    [pki_scores_pred_aligned, mdn_score_pred_align_corrected], dim=0
                )
                # except:
                #     continue
        pki_scores_pred = pki_scores_pred.view(-1).cpu().numpy().tolist()
        pki_scores_pred_ff = pki_scores_pred_ff.view(-1).cpu().numpy().tolist()
        pki_scores_pred_aligned = (
            pki_scores_pred_aligned.view(-1).cpu().numpy().tolist()
        )
        data = zip(
            pdb_ids, pki_scores_pred, pki_scores_pred_ff, pki_scores_pred_aligned
        )  # pki_scores_pred_ff, pki_scores_pred_aligned
