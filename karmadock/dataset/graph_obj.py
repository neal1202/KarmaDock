...
from .ligand_feature import get_ligand_feature_v1
from .protein_feature import get_protein_feature_mda
...
class PDBBindGraphDataset(Dataset):
    ...
    def _generate_graph(self):
        if os.path.exists(self.dst_file):
            ...
            if self.verbose:
                print("### cal graph")
            single_process = partial(
                self._single_process, return_graph=True, save_file=False
            )
            # generate graph
            if self.n_job == 1:
                if self.verbose:
    ...
    def _generate_graph_on_the_fly(self):
        ...
        single_process = partial(
            self._single_process, return_graph=False, save_file=True
        )
        # generate graph
        if self.n_job == 1:
            if self.verbose:
    ...
    def _generate_graph_on_the_fly_fake(self):
        ...
        single_process = partial(
            self._single_process, return_graph=True, save_file=True
        )
        # generate graph
        if self.n_job == 1:
            if self.verbose:
...
class MultiComplexGraphDataset_Fly(Dataset):
    ...
    def _single_process(self, idx):
        ...
        if protein_name not in self.protein2data_dict.keys():
            ...
            pocket_center = torch.tensor(
                get_pocker_center_from_cmd(cry_ligand_mol2), dtype=torch.float32
            )
            # generate graph
    ...
    def __getitem__(self, idx):
        try:
            ...
            data["ligand"].pos = random_rotation(
                shuffle_center(
                    move2center(data["ligand"].pos, pocket_center=data.pocket_center)
                )
            )
        except:
            # pass
...
class VSTestGraphDataset_Fly_SMI(VSTestGraphDataset_Fly):
    ...
    def _single_process(self, idx):
        ...
        data["ligand"].pos = (
            data["ligand"].xyz + data.pocket_center - data["ligand"].xyz.mean(dim=0)
        ).to(torch.float32)
        return data
...
class VSTestGraphDataset_FlyReload_SMI(VSTestGraphDataset_Fly):
    ...
    def __init__(self, protein_file, ligand_path, pocket_center):
        ...
        if not os.path.exists(ligand_path):
            ...
            self.ligand_names = [
                ligand_file.split(".")[0] for ligand_file in os.listdir(ligand_path)
            ]
    ...
    def generate_graphs(self, ligand_smi, n_job=-1, verbose=True):
        ...
        print("reinitialize")
        self.ligand_names = [
            ligand_file.split(".")[0] for ligand_file in os.listdir(self.graph_dir)
        ]
    ...
    def _single_process(self, idx):
        ...
        try:
            ...
            ligand_data["ligand"].pos = (
                ligand_data["ligand"].xyz
                + self.pocket_center
                - ligand_data["ligand"].xyz.mean(dim=0)
            ).to(torch.float32)
...
class VSTestGraphDataset_Fly_SDFMOL2_Refined(VSTestGraphDataset_Fly):
    ...
    def __init__(self, protein_file, ligand_path, pocket_center):
        ...
        self.ligand_names = list(
            set([i.split("_")[0] for i in os.listdir(ligand_path)])
        )
...
class VSTestGraphDataset_Fly_SDFMOL2(VSTestGraphDataset_Fly_SDFMOL2_Refined):
    ...
    def __init__(
        ...
        ligand_path,
        pocket_center,
        geometric_pos_init=True,
        use_rdkit_pos=True,
    ):
        super().__init__(
            protein_file, ligand_path, pocket_center, geometric_pos_init, use_rdkit_pos
        )
...
def get_repeat_node(src_num, dst_num):
    return torch.arange(src_num, dtype=torch.long).repeat(dst_num), torch.as_tensor(
        np.repeat(np.arange(dst_num), src_num), dtype=torch.long
    )
...
def generate_graph_4_Multi_PL(pocket_mol, ligand_mol, use_rdkit_pos=True):
    ...
    (
        ...
        p_edge_v,
        p_full_edge_s,
    ) = get_protein_feature_mda(pocket_mol)
    (
        l_xyz_rdkit,
        l_node_feature,
        l_edge_index,
        l_edge_feature,
        l_full_edge_s,
        l_interaction_edge_mask,
        l_cov_edge_mask,
    ) = get_ligand_feature_v1(rdkit_mol)
    # to data
    data = HeteroData()
    # protein
...
def generate_lig_graph(data, ligand_mol):
    ...
    (
        ...
        l_interaction_edge_mask,
        l_cov_edge_mask,
    ) = get_ligand_feature_v1(ligand_mol)
    # ligand
    data["ligand"].xyz = l_xyz.to(torch.float32)
    data["ligand"].node_s = l_node_feature.to(torch.int32)
    data["ligand"].cov_edge_mask = l_cov_edge_mask
...
def merge_pro_lig_graph(pro_data, data):
    ...
    data["protein", "p2p", "protein"].edge_index = pro_data[
        "protein", "p2p", "protein"
    ].edge_index
    data["protein", "p2p", "protein"].edge_s = pro_data[
        "protein", "p2p", "protein"
    ].edge_s
    data["protein", "p2p", "protein"].full_edge_s = pro_data[
        "protein", "p2p", "protein"
    ].full_edge_s
    data["protein", "p2p", "protein"].edge_v = pro_data[
        "protein", "p2p", "protein"
    ].edge_v
    return data
...
def get_pocker_center_from_cmd(ligand_mol2):
    x = os.popen(
        "cat %s | sed -n '/@<TRIPOS>ATOM/,/@<TRIPOS>BOND/'p | awk '{print $3}' | awk '{x+=$1} END {print x/(NR-2)}'"
        % ligand_mol2
    ).read()
    y = os.popen(
        "cat %s | sed -n '/@<TRIPOS>ATOM/,/@<TRIPOS>BOND/'p | awk '{print $4}' | awk '{y+=$1} END {print y/(NR-2)}'"
        % ligand_mol2
    ).read()
    z = os.popen(
        "cat %s | sed -n '/@<TRIPOS>ATOM/,/@<TRIPOS>BOND/'p | awk '{print $5}' | awk '{z+=$1} END {print z/(NR-2)}'"
        % ligand_mol2
    ).read()
...
def get_mol2_xyz_from_cmd(ligand_mol2):
    x = (
        os.popen(
            "cat %s | sed -n '/@<TRIPOS>ATOM/,/@<TRIPOS>BOND/'p | awk '{print $3}'"
            % ligand_mol2
        )
        .read()
        .splitlines()[1:-1]
    ...
    y = (
        os.popen(
            "cat %s | sed -n '/@<TRIPOS>ATOM/,/@<TRIPOS>BOND/'p | awk '{print $4}'"
            % ligand_mol2
        )
        .read()
        .splitlines()[1:-1]
    ...
    z = (
        os.popen(
            "cat %s | sed -n '/@<TRIPOS>ATOM/,/@<TRIPOS>BOND/'p | awk '{print $5}'"
            % ligand_mol2
        )
        .read()
        .splitlines()[1:-1]
